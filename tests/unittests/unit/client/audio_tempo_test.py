...
class TestTempoThresholds(unittest.TestCase):
    ...
    def test_threshold_values_target_100(self):
        # target=100: lower=75, higher=max(100, 115)=115, fast=460
        target = 100
        lower = target * 3 // 4
        higher = max(target, lower + 40)
        self.assertEqual(lower, 75)
        self.assertEqual(higher, 115)
        self.assertEqual(higher * 4, 460)

    def test_threshold_values_target_40(self):
        # target=40: lower=30, higher=max(40, 70)=70, fast=280
        target = 40
        lower = target * 3 // 4
        higher = max(target, lower + 40)
        self.assertEqual(lower, 30)
        self.assertEqual(higher, 70)
        self.assertEqual(higher * 4, 280)
...
class TestComputeTempoNormal(unittest.TestCase):
    ...
    def test_level_between_limits(self):
        # level=80, target=100: lower=75, higher=115 → between → normal
    ...
    def test_level_just_below_higher(self):
        # level=115, target=100: higher=115 → at limit → still normal
        self.assertEqual(compute_tempo(115, 100, 1.0, 10), TEMPO_NORMAL)
...
class TestComputeTempoAccelerate(unittest.TestCase):
    ...
    def test_level_above_higher(self):
        # level=150, target=100: higher=115 → above → speed up
        self.assertEqual(compute_tempo(150, 100, 1.0, 10), TEMPO_ACCELERATE)

    def test_level_just_below_fast_limit(self):
        # level=460, target=100: fast=460 → at limit → regular accelerate
        self.assertEqual(compute_tempo(460, 100, 1.0, 10), TEMPO_ACCELERATE)
...
class TestComputeTempoFastAccelerate(unittest.TestCase):
    ...
    def test_level_at_fast_limit(self):
        # level=461, target=100: fast=460 → above limit → fast accelerate
        self.assertEqual(compute_tempo(461, 100, 1.0, 10), TEMPO_FAST_ACCELERATE)
...
class TestComputeTempoEdgeCases(unittest.TestCase):
    ...
    def test_target_below_minimum(self):
        # target below TEMPO_MIN_TARGET_MS (40ms) → always normal
        self.assertEqual(compute_tempo(10, 20, 1.0, 10), TEMPO_NORMAL)
        self.assertEqual(compute_tempo(0, 30, 1.0, 10), TEMPO_NORMAL)
        self.assertEqual(compute_tempo(100, TEMPO_MIN_TARGET_MS - 1, 1.0, 10), TEMPO_NORMAL)

    def test_target_at_minimum(self):
        # target=40: lower=30, higher=max(40, 50)=50
        # level=25 → below lower → slow down
        self.assertEqual(compute_tempo(25, TEMPO_MIN_TARGET_MS, 1.0, 10),
                         TEMPO_PREEMPTIVE_EXPAND)

    def test_emergency_override_no_accelerate_while_empty(self):
        # buffer empty + currently accelerating → override to slow down
        # regardless of cooldown:
        self.assertEqual(compute_tempo(0, 100, TEMPO_ACCELERATE, 1),
                         TEMPO_PREEMPTIVE_EXPAND)
        self.assertEqual(compute_tempo(0, 100, TEMPO_FAST_ACCELERATE, 0),
                         TEMPO_PREEMPTIVE_EXPAND)

    def test_emergency_override_not_from_normal(self):
        # buffer empty at normal tempo — regular logic applies (with cooldown):
        self.assertEqual(compute_tempo(0, 100, TEMPO_NORMAL, 1),
                         TEMPO_NORMAL)  # cooldown blocks change
