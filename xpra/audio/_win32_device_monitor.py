...
# ABOUTME: Avoids comtypes COMObject which crashes in frozen builds.
...
# COM vtable function types for IMMNotificationClient
...
_ON_STATE = WINFUNCTYPE(HRESULT, c_void_p, LPCWSTR, DWORD)
_ON_ADDED = WINFUNCTYPE(HRESULT, c_void_p, LPCWSTR)
_ON_REMOVED = WINFUNCTYPE(HRESULT, c_void_p, LPCWSTR)
_ON_DEFAULT = WINFUNCTYPE(HRESULT, c_void_p, ctypes.c_uint, ctypes.c_uint, LPCWSTR)
_ON_PROPERTY = WINFUNCTYPE(HRESULT, c_void_p, LPCWSTR, c_void_p)

# IMMDeviceEnumerator vtable function types (caller side)
_ENUM_REGISTER = WINFUNCTYPE(HRESULT, c_void_p, c_void_p)
_ENUM_UNREGISTER = WINFUNCTYPE(HRESULT, c_void_p, c_void_p)
...
def start(on_change) -> None:
    ...
    # RegisterEndpointNotificationCallback is vtable index 6
    # IUnknown(3) + EnumAudioEndpoints(3) + GetDefaultAudioEndpoint(4) + GetDevice(5) + Register(6)
